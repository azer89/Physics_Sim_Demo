...
void OgreBulletListener::bulletShutdown()
{
    ...
    while (mEntities.end() != itEntity)
    {
        ...
        mBulletSceneMgr->destroyEntity(*itEntity);
...
//{
    ...
    mBulletWorld->launchRay (*mCollisionClosestRayResultCallback);
    ...
    case KC_B: 
        if ( checkIfEnoughPlaceToAddObject(trowDist))
        {
            ...
            body->setLinearVelocity(
                mBulletCamera->getDerivedDirection().normalisedCopy() * mShootSpeed
				);
		}
        break;
    case KC_N: 
...
{
    // Start Bullet
    mBulletWorld = new DynamicsWorld (mBulletSceneMgr, bounds, gravityVector);
...
{
    Entity *entity = mBulletSceneMgr->createEntity(
                                              ...
                                              const Ogre::Real bodyFriction)
{
    ...
    int i = 0;
    StaticGeometry* s;
    s = mBulletSceneMgr->createStaticGeometry("StaticFloor");
    ...
    for (Real z = -80.0;z <= 80.0;z += 20.0)
    {
        for (Real x = -80.0;x <= 80.0;x += 20.0)
        {
            ...
            Entity* entity = mBulletSceneMgr->createEntity(name, "plane.mesh");
